...
vector<Node> allocList,freeList;
...
bool malloc_firstFit(int mem[],int block_size,int block_number){
    int count=0;
    Node temp;
    for (auto i = freeList.begin(); i != freeList.end(); ++i){
        temp=*i;
        ...
        if(temp.size==(*freeList.end()).size && temp.size<block_size)
            return false;
        count++;
    }
    int rem=block_size%Threshold;
...
bool malloc_nextFit(int mem[],int block_size,int block_number){
    static auto i=freeList.begin();
    if(i==freeList.end()){
        i=freeList.end();
    }
    int count=0;
    Node temp;
    for (i; i != freeList.end(); ++i){
        temp=*i;
        if(temp.size>=block_size){
            break;
        }
...
bool freeBlock(int blockNumber,int mem[]){
    int index,count=0;
    Node temp,current,previous,next;
    for (auto i = allocList.begin(); i != allocList.end(); ++i){
        ...
        if(i==allocList.end() && temp.blockNumber!=(*allocList.end()).blockNumber){
            return false;
        }
        count++;
    }
    ...
    if((*freeList.begin()).startAddress==MEM_SIZE){
        ...
        sortList(freeList);
        memset(mem+current.startAddress,0,current.size*sizeof(int));
    }
    else{
        for (auto i = freeList.begin(); i != freeList.end(); ++i){
            temp=*i;
            if(temp.startAddress> current.startAddress){
                break;
            }
            count++;
        }
